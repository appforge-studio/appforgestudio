...
class PropertyEditorController extends GetxController {
  ...
  void selectComponent(ComponentModel? component) {
    debugPrint('🎛️ PropertyEditor: selectComponent called with ${component?.id}');
  ...
  void updateComponentProperties(ComponentModel component, ComponentProperties newProperties) {
  ...
  void updatePropertyValue(String propertyKey, dynamic value) {
    ...
    final updatedProperties = component.properties.updateProperty(propertyKey, value);
    final updatedComponent = component.copyWith(properties: updatedProperties);
    _updateComponentAndNotifyCanvas(updatedComponent);
  }
  
  // Property validation methods
  bool validateComponentProperties(ComponentProperties properties, ComponentType type) {
    switch (type) {
      case ComponentType.container:
        final width = properties.getProperty<double>('width') ?? 0;
        final height = properties.getProperty<double>('height') ?? 0;
        final borderRadius = properties.getProperty<double>('borderRadius') ?? 0;
        return width > 0 && height > 0 && borderRadius >= 0;
      case ComponentType.text:
        final content = properties.getProperty<String>('content') ?? '';
        final fontSize = properties.getProperty<double>('fontSize') ?? 0;
        return content.isNotEmpty && fontSize > 0;
      case ComponentType.image:
        final source = properties.getProperty<String>('source') ?? '';
        final width = properties.getProperty<double>('width') ?? 0;
        final height = properties.getProperty<double>('height') ?? 0;
        final borderRadius = properties.getProperty<double>('borderRadius') ?? 0;
        return source.isNotEmpty && width > 0 && height > 0 && borderRadius >= 0;
    }
  }
  
  // Get default properties for each component type
  ComponentProperties getDefaultContainerProperties() {
    return ContainerProperties.createDefault();
  }
  
  ComponentProperties getDefaultTextProperties() {
    return TextProperties.createDefault();
  }
  
  ComponentProperties getDefaultImageProperties() {
  ...
  ComponentProperties getDefaultPropertiesForType(ComponentType type) {
    switch (type) {
      case ComponentType.container:
        return getDefaultContainerProperties();
      case ComponentType.text:
  ...
  void fromJson(Map<String, dynamic> json) {
    final selectedComponentJson = json['selectedComponent'] as Map<String, dynamic>?;
    ...
    _updateState(PropertyEditorState(
  ...
  void onInit() {
    ...
    ever(_canvasController.stateStream, (CanvasState canvasState) {
      debugPrint('🎛️ PropertyEditor: Canvas state changed - selectedComponent: ${canvasState.selectedComponent?.id}');
      if (canvasState.selectedComponent != _state.value.selectedComponent) {
        debugPrint('🎛️ PropertyEditor: Updating selection to ${canvasState.selectedComponent?.id}');
