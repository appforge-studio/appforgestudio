...
String formatPathD(List<PathNode> nodes, bool closed) {
  ...
  for (int i = 0; i < nodes.length; i++) {
    ...
    // First node or explicit move
    if (i == 0 || curr.isMove) {
      // If not first node, and we were closed, maybe close previous?
      // For now we trust implicit closure or separate logic if needed.
      // Actually, if we hit a Move, we should close previous if it was a loop?
      // Let's just output M.
      if (i > 0 && closed) {
        // Basic close for previous subpath if requested globally?
        // SVG d string structure usually has Z at end of subpath.
        // We can check if previous subpath started with M...
        // Ideally 'closed' property should be per subpath or we infer it.
        // For simple icon usage, we often assume Z at very end or for all subpaths.
        d += ' Z';
      }
      d +=
          ' ${d.isEmpty ? '' : ' '}M ${curr.position.dx.toStringAsFixed(2)} ${curr.position.dy.toStringAsFixed(2)}';
      continue;
    }
    ...
    if (prev.isCurve || curr.isCurve) {
      d +=
          ' C ${prev.handleOut.dx.toStringAsFixed(2)} ${prev.handleOut.dy.toStringAsFixed(2)}, '
      ...
      d +=
          ' L ${curr.position.dx.toStringAsFixed(2)} ${curr.position.dy.toStringAsFixed(2)}';
    }
  }

  if (closed && nodes.isNotEmpty) {
    d += ' Z';
  }
...
List<PathNode> parseSVGPath(String d) {
  ...
  double nextNum() {
    ...
    debugPrint(
      'WARNING: Expected number but got "${tokens[cursor]}" at cursor $cursor',
    );
    return 0;
  }
