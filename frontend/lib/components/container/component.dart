...
class ContainerComponent extends ComponentModel {
  ...
  Map<String, dynamic> get jsonSchema {
    ...
    final applyBorder = properties.shouldApplyProperty('border');
    final borderColor =
        properties.getProperty<XDColor>('border') ?? XDColor(['#000000']);
    ...
    final backgroundBlurOpacity =
        properties.shouldApplyProperty('backgroundBlurOpacity')
        ? (properties.getProperty<double>('backgroundBlurOpacity') ?? 1.0)
        : 1.0;

    // Apply opacity to background color if blur is enabled
    if (backgroundBlur > 0 && applyBg) {
      if (backgroundColor.type == ColorType.solid) {
        final color = backgroundColor.toColor();
        final modifiedColor = color.withOpacity(
          color.opacity * backgroundBlurOpacity,
        );
        colorHex =
            '#${modifiedColor.toARGB32().toRadixString(16).padLeft(8, '0')}';
      } else if (gradient != null) {
        // Apply opacity to gradient colors
        final colors = backgroundColor.toColors().map((c) {
          final modifiedColor = c.withOpacity(
            c.opacity * backgroundBlurOpacity,
          );
          return '#${modifiedColor.toARGB32().toRadixString(16).padLeft(8, '0').toUpperCase()}';
        }).toList();
